var alignmentFns = {
    ...
    init: function(input, step, max, min, digit) {
        ...
        //添加左右加减号
        input.wrap("<div id = '" + input.attr('id') + "div' style = 'overflow:hidden;width:112px; height:30px;border: 1px solid #e6e6e6; margin-left:30px'></div>");
        input.before("<div id = '" + input.attr('id') + "l'  onselectstart = 'return false;' style = '-moz-user-select:none;cursor:pointer;text-align:center;width:30px; height:32px;line-height:32px;border-right: 1px solid #e6e6e6;float:left;background:#f5f5f5'>-</div>");
        input.after("<div id = '" + input.attr('id') + "r'  onselectstart = 'return false;' style = '-moz-user-select:none;cursor:pointer;text-align:center;width:30px; height:32px;line-height:32px;border-left: 1px solid #e6e6e6;float:right;background:#f5f5f5'> + </div>");
        //左右调用执行
